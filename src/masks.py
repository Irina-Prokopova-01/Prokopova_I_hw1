...
def get_mask_card_number(card_number: str) -> str | None:
    ...
    if type(card_number) is not str:
        logger.error(f'Передан не корректный тип данных {card_number}.')
        raise TypeError
    """Функция маскирующая номер карты"""
    if card_number.isdigit() and len(card_number) == 16:
        logger.warning(f'Введеный номер должен состоять из 16 цифр {card_number}')
...
def get_mask_account(bank_account: str) -> str | None:
    ...
    if type(bank_account) is not str:
        logger.error(f"Передан не корректный тип данных {bank_account}.")
        raise TypeError
    """Функция маскирующая номер счета"""
    if bank_account.isdigit() and len(bank_account) == 20:
        logger.warning(f"Введеный номер должен состоять из 20 цифр {bank_account}")
